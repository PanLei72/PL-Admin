...
app.controller('ListController', function($scope, $resource,$stateParams,$modal,$state) {
    ...
    $scope.search = function(){
        $state.go('app.system.list',{search:$scope.search_context});
    }
    //全选
    var selected = false;
        ...
        if($scope.operate=="1"){
            ...
            if(ids.length>0){
                ...
                modalInstance.result.then(function () {
                      var $com = $resource($scope.app.host + "/system/deletes/?");
...
app.controller('DetailController', function($rootScope,$scope, $resource, $stateParams,$state) {
  ...
  if($scope.edit_mode){
      var $com = $resource($scope.app.host + "/system/:id/?",{id:'@id'});
...
//          var $com = $resource($scope.app.host + "/system/?");
